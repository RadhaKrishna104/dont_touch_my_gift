...
class RightHand(Sprite):
    def __init__(self, screen_width, screen_height, offset_from_ground=200):
        ...
        #delay hands
        self.hands_can_move = False  # Initially, hands cannot move
        self.start_time = pygame.time.get_ticks()  # Record the game start time
    ...
    def update(self):
        ...
        if not self.hands_can_move and current_time - self.start_time >= 5000:
            self.hands_can_move = True  # Allow hands to move after the delay
        
         # Move hands only if the flag is set
        if self.hands_can_move:
